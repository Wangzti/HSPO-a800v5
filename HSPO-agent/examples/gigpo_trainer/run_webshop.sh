...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-/root/autodl-tmp/checkpoints/verl_agent_webshop/gigpo_qwen2.5_1.5b}"
...
# Keep the Ray CPU claim fractional so env actors do not starve the GPU worker placement group.
num_cpus_per_env_worker="${NUM_CPUS_PER_ENV_WORKER:-0.05}"

# WebShop workers are much heavier than ALFWorld workers (~0.6GB RSS each).
# Keep defaults conservative: train_batch_size * group_size + val_batch_size = 24 workers.
