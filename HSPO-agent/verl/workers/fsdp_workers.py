...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def update_actor_hspo(self, data: DataProto):
        """HSPO token-level credit-routed PPO update.

        Computes fresh log-probs inside the worker (with gradient tracking),
        applies HSPO token-level loss, then backward + optimizer step.
        """
        from recipe.hspo.core_hspo import HSPOConfig, compute_hspo_loss

        hspo_cfg_dict = data.meta_info.pop("hspo_cfg_dict")
        phase = data.meta_info.pop("hspo_phase")
        use_kl = data.meta_info.pop("hspo_use_kl")
        ...
        hspo_cfg = HSPOConfig(**hspo_cfg_dict)
        effective_use_kl = use_kl and "ref_log_prob" in data.batch
        ...
        with self.ulysses_sharding_manager:
            ...
            select_keys = [
                ...
                "old_log_probs", "advantages_low", "advantages_high",
                "action_mask", "subgoal_mask", "switch_mask", "response_mask",
            ]
            if effective_use_kl:
            ...
            for _ in range(self.config.actor.ppo_epochs):
                for mini_batch in mini_batches:
                    grad_accum = self.config.actor.ppo_mini_batch_size // self.config.actor.ppo_micro_batch_size_per_gpu
                    micro_batches = mini_batch.split(self.config.actor.ppo_micro_batch_size_per_gpu)
                    ...
                    for micro_batch in micro_batches:
                        ...
                        advantages_low = micro_batch.get("advantages_low")
                        advantages_high = micro_batch.get("advantages_high")
                        action_mask = micro_batch.get("action_mask")
                        subgoal_mask = micro_batch.get("subgoal_mask")
                        switch_mask = micro_batch.get("switch_mask")
                        response_mask = micro_batch["response_mask"]
                        ref_log_prob = micro_batch.get("ref_log_prob") if effective_use_kl else None

                        loss, batch_metrics = compute_hspo_loss(
                            log_probs=log_probs,
                            old_log_probs=old_log_probs,
                            advantages_low=advantages_low,
                            advantages_high=advantages_high,
                            action_mask=action_mask,
                            subgoal_mask=subgoal_mask,
                            switch_mask=switch_mask,
                            response_mask=response_mask,
                            ref_log_prob=ref_log_prob,
                            use_kl=effective_use_kl,
                            cfg=hspo_cfg,
                            phase=phase,
                        )
                    ...
                    metrics["actor/grad_norm"] = grad_norm.item() if torch.is_tensor(grad_norm) else grad_norm
